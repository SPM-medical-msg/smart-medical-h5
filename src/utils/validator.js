...
// 30、验证手机号是否已注册
export const isPhoneRegistered = async (rule, value, callback) => {
  if (!value) {
  ...
  try {
    // 将字符串包装成对象
    const response = await getUserByPhone({ phone: value });
    ...
    // 网络错误时不阻止表单提交，或根据需求处理
    callback();
  }
};

// 31、验证用户名是否已存在
export const isUserNameExist = async (rule, value, callback) => {
  if (!value) {
